# SAMPR post-deploy e2e smoke tests
...
$app = Join-Path $root "build\SAMPR_artefacts\Release\SAMPR.exe"
$tests = Join-Path $root "build\SAMPR_Tests_artefacts\Release\SAMPR_Tests.exe"
$failures = @()
$assistantProc = $null
...
function Stop-AssistantIfRunning {
    ...
    Get-Process node -ErrorAction SilentlyContinue | Where-Object {
        try { $_.Path -like "*\sampr\assistant\*" } catch { $false }
...
Write-Host " SAMPR Post-Deploy E2E Smoke Tests" -ForegroundColor Yellow
Write-Host "========================================" -ForegroundColor Yellow
...
if (Test-Path $app) {
    ...
    if ($size -gt 1MB) { Pass "SAMPR.exe exists ($([math]::Round($size/1MB,1)) MB)" }
    else { Fail "SAMPR.exe too small ($size bytes)" }
} else {
    Fail "SAMPR.exe missing"
}
...
# 5) SAMPR process launch smoke
Info "Phase 5: SAMPR process launch"
if (Test-Path $app) {
    try {
        $proc = Start-Process -FilePath $app -WorkingDirectory (Split-Path $app) -PassThru
        Start-Sleep -Seconds 3
        if (-not $proc.HasExited) {
            Pass "SAMPR.exe launched and stayed alive (pid $($proc.Id))"
            Stop-Process -Id $proc.Id -Force
        } else {
            Fail "SAMPR.exe exited early with code $($proc.ExitCode)"
        }
    } catch {
        Fail "SAMPR launch error: $($_.Exception.Message)"
    }
} else {
    Fail "Cannot launch - SAMPR.exe missing"
}
