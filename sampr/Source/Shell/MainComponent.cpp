...
bool MainComponent::isInterestedInFileDrag (const juce::StringArray& files)
{
    for (const auto& path : files)
        if (sampr::SampleImportService::isSupportedAudioFile ({ path }))
            return true;
...
{
    undoButton.setEnabled (projectController.canUndo());
    redoButton.setEnabled (projectController.canRedo());
}
...
{
    g.fillAll (sampr::SamprLookAndFeel::background());
...
void MainComponent::resized()
{
    ...
    topNavBar.setBounds (area.removeFromTop (28));

    auto projectRow = area.removeFromTop (26);
    ...
    statusLabel.setBounds (area.removeFromBottom (40));
    area.removeFromBottom (4);
    transportBar.setBounds (area.removeFromBottom (30));
    area.removeFromBottom (4);
    mixerPanel.setBounds (area.removeFromBottom (juce::jmax (164, 220)));
    area.removeFromBottom (4);

    sampleBrowser.setBounds (area.removeFromLeft (180));
    area.removeFromLeft (4);
    centralEditor.setBounds (area);

    auto editor = centralEditor.getLocalBounds();
    const auto chopHeight = juce::jmax (132, editor.getHeight() / 3);
...
void MainComponent::saveBeatToDisk()
{
    ...
    fileChooser->launchAsync (chooserFlags, [this] (const juce::FileChooser& chooser)
    {
        ...
        if (! file.hasFileExtension ("sampr-beat.json"))
            file = file.withFileExtension (".sampr-beat.json");
...
void MainComponent::exportAudioToDisk()
{
    ...
    fileChooser->launchAsync (chooserFlags, [this] (const juce::FileChooser& chooser)
    {
        ...
        const auto result = sampr::OfflineExporter::exportToWav (
            file, audioEngine, patternStore, projectController, projectModel, options);
