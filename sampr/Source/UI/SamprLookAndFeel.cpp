...
namespace
{
    ...
    juce::Colour buttonFill (const juce::Button& button, bool highlighted, bool down)
    {
        ...
        if (button.getToggleState())
            return SamprLookAndFeel::accent().withAlpha (0.22f);
                                             ...
                                             bool shouldDrawButtonAsDown)
{
    ...
    g.setColour (buttonFill (button, shouldDrawButtonAsHighlighted, shouldDrawButtonAsDown));
    g.fillRect (bounds);
    g.setColour (border());
                                    ...
                                    bool)
{
    ...
    g.setColour (isEnabled ? surface() : surface().darker (0.2f));
    g.fillRect (box);
    g.setColour (border());
    g.drawRect (box, 1.0f);
                                      ...
                                      bool isMouseDown)
{
    ...
    g.setColour (isFront ? panel() : background());
    g.fillRect (area);
                                     ...
                                     juce::ComboBox& box)
{
    ...
    g.setColour (isButtonDown ? divider() : surface());
    g.fillRect (bounds);
    g.setColour (border());
    g.drawRect (bounds, 1.0f);
...
{
    return juce::FontOptions { juce::jmin (13.0f, static_cast<float> (buttonHeight) * 0.55f) };
}
