    ...
    : sampleManager (manager)
{
    ...
    openFxButton.setTooltip ("Open the FX rack for the sequencer channel using this slice");
    openFxButton.onClick = [this]
    {
        if (openFxCallback != nullptr)
            openFxCallback();
    };
    ...
    addAndMakeVisible (openFxButton);
...
void SliceEditorPanel::setOpenFxCallback (OpenFxCallback callback)
{
    openFxCallback = std::move (callback);
}
...
void SliceEditorPanel::resized()
{
    ...
    autoSliceButton.setBounds (actionRow.removeFromLeft (actionRow.getWidth() / 2 - 4));
    actionRow.removeFromLeft (8);
    openFxButton.setBounds (actionRow);
    area.removeFromTop (8);
