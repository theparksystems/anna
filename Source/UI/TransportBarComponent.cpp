...
TransportBarComponent::TransportBarComponent()
{
    ...
    soundCloudButton.setTooltip ("Export a SoundCloud-ready WAV to the upload folder");
    ...
    soundCloudButton.onClick = [this] { if (onSoundCloudExport) onSoundCloudExport(); };
...
void TransportBarComponent::setSoundCloudExportCallback (SoundCloudExportCallback callback) { onSoundCloudExport = std::move (callback); }
...
void TransportBarComponent::paint (juce::Graphics& g)
{
    ...
    g.setGradientFill (juce::ColourGradient (SamprLookAndFeel::transport().brighter (0.06f),
                                             0.0f, 0.0f,
                                             SamprLookAndFeel::transport().darker (0.14f),
                                             0.0f, static_cast<float> (bounds.getBottom()),
                                             false));
    g.fillRect (bounds);
    g.setColour (SamprLookAndFeel::accent().withAlpha (0.16f));
    g.fillRect (0, bounds.getBottom() - 2, bounds.getWidth(), 1);
...
void TransportBarComponent::resized()
{
    ...
    row.items.add (juce::FlexItem (playButton).withWidth (44).withHeight (30).withMargin (margin));
    row.items.add (juce::FlexItem (pauseButton).withWidth (44).withHeight (30).withMargin (margin));
    row.items.add (juce::FlexItem (stopButton).withWidth (44).withHeight (30).withMargin (margin));
    row.items.add (juce::FlexItem (recordButton).withWidth (78).withHeight (30).withMargin (margin));
    row.items.add (juce::FlexItem (exportButton).withWidth (96).withHeight (30).withMargin (margin));
    row.items.add (juce::FlexItem (soundCloudButton).withWidth (82).withHeight (30).withMargin (margin));
