                                ...
                                AudioDiagnostics& diagnostics) noexcept
{
    ...
    const auto samplesPerStep = snap->samplesPerStep;
    const auto firstPatternPosition = blockStartSamplePosition % patternLengthSamples;
    auto stepIndex = static_cast<int> (static_cast<double> (firstPatternPosition) / samplesPerStep);
    auto stepStart = static_cast<uint64_t> (static_cast<double> (stepIndex) * samplesPerStep);

    if (stepStart < firstPatternPosition)
    {
        ++stepIndex;

        if (stepIndex >= snap->numSteps)
            stepIndex = 0;
    }

    auto patternBase = blockStartSamplePosition - firstPatternPosition;
    auto nextStepPosition = patternBase + static_cast<uint64_t> (static_cast<double> (stepIndex) * samplesPerStep);

    if (nextStepPosition < blockStartSamplePosition)
        nextStepPosition += patternLengthSamples;

    while (nextStepPosition < blockEnd)
    {
        const auto loopCycle = nextStepPosition / patternLengthSamples;
        ...
        if (stepIndex + 1 < snap->numSteps)
        {
            ++stepIndex;
            nextStepPosition = patternBase + static_cast<uint64_t> (static_cast<double> (stepIndex) * samplesPerStep);
        }
        else
        {
            stepIndex = 0;
            patternBase += patternLengthSamples;
            nextStepPosition = patternBase;
        }
    }
}
