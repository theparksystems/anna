...
namespace
{
    ...
    SampleOriginMetadata loadOriginSidecar (const juce::File& audioFile)
    {
        SampleOriginMetadata origin;
        origin.sourceType = "local-file";
        origin.localFileName = audioFile.getFileName();

        auto sidecar = audioFile.getSiblingFile (audioFile.getFileNameWithoutExtension() + ".anna-origin.json");
        if (! sidecar.existsAsFile())
            sidecar = audioFile.getSiblingFile (audioFile.getFileNameWithoutExtension() + ".sampr-origin.json");

        if (! sidecar.existsAsFile())
            return origin;

        juce::var parsed;
        const auto parseError = juce::JSON::parse (sidecar.loadFileAsString(), parsed);

        if (parseError.failed() || ! parsed.isObject())
            return origin;

        origin.sourceType = parsed.getProperty ("sourceType", origin.sourceType).toString();
        origin.sourceUrl = parsed.getProperty ("sourceUrl", juce::String()).toString();
        origin.sourceTitle = parsed.getProperty ("sourceTitle", juce::String()).toString();
        origin.sourceAuthor = parsed.getProperty ("sourceAuthor", juce::String()).toString();
        origin.sourceId = parsed.getProperty ("sourceId", juce::String()).toString();
                                                    ...
                                                    juce::AudioFormatManager& formatManager)
{
    ...
    if (reader == nullptr)
    {
        DBG ("ANNA: failed to create reader for " + file.getFullPathName());
        return std::nullopt;
    }
    ...
    {
        DBG ("ANNA: failed to decode " + file.getFullPathName());
        return std::nullopt;
    }
    ...
    asset.origin = loadOriginSidecar (file);
    ...
    DBG ("ANNA: loaded asset '" + file.getFileName() + "' id=" + juce::String (assetId));
    notifyChanged();
    return assetId;
}
...
void SampleManager::requestSliceBake (AssetId assetId, int sliceIndex)
{
    ...
    {
        DBG ("ANNA: Rubber Band not available; using raw playback-rate fallback");
        notifyChanged();
        return;
    }
...
void SampleManager::handleBakeResult (const StretchBakeResult& result)
{
    ...
    {
        DBG ("ANNA: stretch bake failed for slice " + juce::String (result.sliceId));
        notifyChanged();
        return;
    }
    ...
    DBG ("ANNA: baked slice " + juce::String (slice->id)
    ...
    juce::AudioFormatManager& formatManager)
{
    ...
    for (const auto& ref : refs)
    {
        ...
        if (! sampleFile.existsAsFile())
        {
            DBG ("ANNA: missing sample for ref " + ref.refId + " -> " + ref.relativePath);
            continue;
        }
        ...
        asset->origin = ref.origin.hasSource() ? ref.origin : asset->origin;
