...
namespace
{
    ...
    void testProjectRoundTrip()
    {
        ...
        const auto tempFile = juce::File::getSpecialLocation (juce::File::tempDirectory)
                                  .getChildFile ("anna_test_project.anna.json");
    ...
    void testFxRoundTrip()
    {
        ...
        const auto tempFile = juce::File::getSpecialLocation (juce::File::tempDirectory)
                                  .getChildFile ("anna_test_fx.anna.json");
    ...
    void testSampleOriginRoundTrip()
    {
        sampr::ProjectModel original;
        sampr::SampleRefData ref;
        ref.refId = "sample-1";
        ref.displayName = "Hook";
        ref.relativePath = "Samples/hook.wav";
        ref.origin.sourceType = "youtube";
        ref.origin.sourceUrl = "https://www.youtube.com/watch?v=test";
        ref.origin.sourceTitle = "Original Song";
        ref.origin.sourceAuthor = "Original Artist";
        ref.origin.sourceId = "test";
        original.getSampleRefs().push_back (ref);

        const auto tempDir = juce::File::getSpecialLocation (juce::File::tempDirectory)
                                  .getChildFile ("anna_origin_roundtrip");
        tempDir.createDirectory();
        const auto tempFile = tempDir.getChildFile ("origin.anna.json");

        juce::String saveError;
        expect (sampr::ProjectSerializer::saveToFile (original, tempFile, saveError),
                "origin project saves");

        const auto loaded = sampr::ProjectSerializer::loadFromFile (tempFile);
        expect (loaded.success, "origin project loads");
        expect (loaded.model.getSampleRefs().front().origin.sourceUrl == ref.origin.sourceUrl,
                "origin url round-trips");
        expect (tempDir.getChildFile ("metadata").getChildFile ("sample_sources.json").existsAsFile(),
                "sample source metadata file is written");
        expect (tempDir.getChildFile ("metadata").getChildFile ("sampled_from.txt").loadFileAsString().contains ("Original Song"),
                "sampled-from list is written");
    ...
    void testBeatRoundTrip()
    {
        ...
        const auto tempFile = juce::File::getSpecialLocation (juce::File::tempDirectory)
                                  .getChildFile ("anna_test_beat.anna-beat.json");
...
int main (int argc, char* argv[])
{
    ...
    if (failures == 0)
    {
        std::cout << "All ANNA unit tests passed.\n";
        return 0;
    }
